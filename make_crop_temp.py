                    ...
                    for (classid, score, box) in zip(classes, scores, boxes):
                        if score >= 0.5:
                            ...
                            if startX <= w and endX <= w and startY <= h and endY <= h:
                                ...
                                count += 1
                                cv2.imwrite(os.path.join(opt.base_dir, "{}_crop_{}.{}".format(partitions[0], count, partitions[1]).replace("Data", "photo_crop")), face)

                    if count == 0:
                        cv2.imwrite(os.path.join(opt.base_dir, "{}_crop_1.{}".format(partitions[0], partitions[1]).replace("Data", "photo_crop")), im)
                    ...
                    try:
                        ...
                        if "train" in im_p:
                            if "spoof" in im_p:
                                ...
                                cv2.imwrite(os.path.join(opt.base_dir, "{}_crop_1.{}".format(partitions[0], partitions[1]).replace("Data", "photo_crop")), im_temp)
                            else:
